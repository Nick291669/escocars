    ...
    try {
      ...
      if (signInError) {
        if (signInError.message === 'Invalid login credentials') {
          ...
          setError('Bitte bestätige zuerst deine E-Mail-Adresse und versuche es danach erneut.')
        } else {
      ...
      setError(getSupabaseRequestError(requestError))
    } finally {
      setLoading(false)
    }
  }
      ...
      <div className="mx-auto max-w-md">
        ...
        <div className="mt-10 rounded-[2rem] border border-white/10 bg-white/[0.035] p-6 shadow-2xl md:p-8">
          ...
          <p className="mt-3 text-sm leading-6 text-zinc-500">
            Melde dich an, um Buchungen zu erstellen und deine Mieten zu verwalten.
          </p>
          ...
          <form onSubmit={submit} className="mt-8 space-y-4">
            <label className="block text-sm text-zinc-300">
              ...
              <input
                ...
                className="mt-2 w-full rounded-xl border border-white/10 bg-black/30 px-4 py-3 outline-none transition focus:border-amber-400/60"
              />
            </label>
            ...
            <button
              ...
              className="w-full rounded-xl bg-amber-400 px-4 py-3.5 font-semibold text-black transition hover:bg-amber-300 disabled:cursor-not-allowed disabled:opacity-50"
            >
          ...
          <p className="mt-6 text-center text-sm text-zinc-500">
            Noch kein Konto?{' '}
            <Link href={`/registrieren?next=${encodeURIComponent(next)}`} className="font-medium text-amber-400 hover:text-amber-300">
              Jetzt registrieren
            </Link>
          </p>
        </div>
...
export default function LoginPage() {
  return (
    <Suspense fallback={<main className="min-h-screen bg-[#070806] text-white" />}>
      <LoginForm />
    </Suspense>
  )
}
